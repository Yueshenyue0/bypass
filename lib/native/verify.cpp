...
// 通过 dart:ffi 调用。编译时用 -DEXPECTED_LIBAPP_SHA256=xxx 注入期望 hash
#include <dlfcn.h>
#include <fcntl.h>
#include <stdint.h>
#include <stdio.h>
...
// 编译时注入：-DEXPECTED_LIBAPP_SHA256="xxxx"
...
static void sha256_final(sha256_ctx* ctx, uint8_t hash[32]) {
  ...
  for (i = 0; i < 8; i++) {
    ctx->data[63 - i] = (uint8_t)(bitlen & 0xff);
    bitlen >>= 8;
  }
  sha256_transform(ctx, ctx->data);
...
// 通过 dladdr 定位自身 so 路径，得到 native 目录（与 libapp.so 同目录）
static bool get_native_dir(std::string* out) {
  Dl_info info;
  if (dladdr((void*)&get_native_dir, &info) == 0 || info.dli_fname == NULL)
    return false;
...
// 返回值：0=未篡改  1=被篡改  -1=校验出错
extern "C" __attribute__((visibility("default"))) int verify_apk_integrity() {
  std::string dir;
  if (!get_native_dir(&dir)) return -1;

  std::string libapp = dir + "libapp.so";
  std::string libflutter = dir + "libflutter.so";

  const std::string expected = EXPECTED_LIBAPP_SHA256;
  if (expected == "__EXPECTED_SHA__") return -1;

  std::string app_hash = sha256_file(libapp.c_str());
  if (app_hash.empty()) return -1;
  if (app_hash != expected) return 1;

  // libflutter.so 必须存在（不存在说明被删了）
  if (sha256_file(libflutter.c_str()).empty()) return 1;
  return 0;
}

// 调试函数：输出当前 libapp.so 的 SHA-256
